<template>
  <el-table class="grid" :data="tableData">
    ...
    <el-table-column prop="role_grp_seq" label="NO" min-width="30"></el-table-column>

    <el-table-column label="그룹명">
      <template slot="header">
        <div>
          <span>그룹명</span>
          <i class="el-icon-d-caret pointer" @click="toggleOrder('role_name_fg')"></i>
        </div>
      </template>
      <template slot-scope="scope">
        <span class="pointer" @click="showAlterPop1(scope.row)">{{ scope.row.role_grp_name }}</span>
      </template>
    </el-table-column>

    <el-table-column prop="role_grp_desc" label="그룹설명">
      <template slot-scope="scope">
        <span>{{ scope.row.role_grp_desc }}</span>
      </template>
    </el-table-column>
  ...
  methods: {
    ...
    handleSelectionChange(index) {
      ...
      if (this.selectedRows[index]) {
        arr.push(this.tableData[index].role_grp_seq);
      } else {
        for(let i = 0; i < arr.length; i++) {
          if(arr[i] === this.tableData[index].role_grp_seq)  {
            arr.splice(i, 1);
            i--;
          }
        }
